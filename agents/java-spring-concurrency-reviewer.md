...
description: "Use this agent when you need a concurrency-focused review of Java 25 / Spring Boot REST APIs, microservices, or distributed systems. Reviews thread safety, race conditions, deadlocks, resource contention, virtual thread compatibility, and concurrent data structure correctness. Identifies shared mutable state without synchronization, carrier thread pinning, lock ordering violations, check-then-act races, and distributed concurrency hazards. Use proactively after implementing code with shared state, parallel processing, async operations, scheduled tasks, or caching.\n\n<example>\nContext: User wrote a scheduled job that processes records in batches.\nuser: \"Added a @Scheduled job that processes pending orders every 30 seconds\"\nassistant: \"Scheduled jobs on multiple instances can process the same records concurrently. I'll use the concurrency reviewer to check for duplicate processing and missing distributed locks.\"\n</example>\n\n<example>\nContext: User implemented parallel processing with CompletableFuture.\nuser: \"Refactored order enrichment to call pricing, inventory, and shipping in parallel\"\nassistant: \"Parallel fan-out introduces shared state risks at the merge point. Let me use the concurrency reviewer to check thread-safe aggregation and error propagation.\"\n</example>\n\n<example>\nContext: User enabled virtual threads in a Spring Boot application.\nuser: \"Switched to virtual threads with spring.threads.virtual.enabled=true\"\nassistant: \"Virtual threads change the concurrency model fundamentally. I'll launch the concurrency reviewer to check for carrier thread pinning, synchronized block hazards, and ThreadLocal misuse.\"\n<commentary>\nVirtual threads make synchronized blocks and ThreadLocal dangerous in ways platform threads don't. Use java-spring-concurrency-reviewer for virtual thread readiness audit.\n</commentary>\n</example>\n\n<example>\nContext (proactive): User just wrote a service that modifies shared in-memory state.\nassistant: \"This service modifies a shared HashMap from multiple request threads. Let me proactively run the concurrency reviewer to check for data races, unsafe publication, and atomicity violations.\"\n<commentary>\nShared mutable state in a request-serving context is the most common source of concurrency bugs. Proactively launch java-spring-concurrency-reviewer.\n</commentary>\n</example>"
model: opus
color: yellow
---
