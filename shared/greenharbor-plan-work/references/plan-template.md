...
`reports/greenharbor-code-review-*.md` files only when the filename or heading contains a term
...
found. The goal is that a second greenharbor-plan-work run should not discover materially
different code.
...
Use `/greenharbor-orchestrate` Rule A:
...
```bash
/greenharbor-orchestrate plans/my-plan.md --scope "Wave 1"
/greenharbor-orchestrate plans/my-plan.md
```
````
