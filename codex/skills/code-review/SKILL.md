...
description: |
  ...
  "code-review", wants a pre-landing check, or needs production-readiness assessment.
  Modes: /code-review (diff), /code-review --scope src/billing/ (scoped),
  /code-review --full (full project), /code-review --flow "POST /api/orders" (endpoint trace).
---

# Code Review

You are running the `/code-review` workflow — a multi-mode code review tool.
...
```
/code-review — Universal code review with actionable triage

Usage:
  /code-review                                    Diff against main (default)
  /code-review --branch develop                   Diff against a specific branch
  /code-review --baseline HEAD~5                  Diff against last 5 commits
  /code-review --scope src/billing/               Review a specific directory
  /code-review --scope OrderService               Review files matching a name
  /code-review --scope "src/billing/**/*.java"    Review files matching a glob
  /code-review --full                             Full project review (decomposed)
  /code-review --flow "POST /api/orders"          Trace and review an endpoint flow
...
/plan-work "<description>" --review {REPORT_PATH}
...
/code-review --scope <ServiceClass> for each service in the chain.
```
...
otherwise auto-generate at `reports/code-review-<mode>-<YYYY-MM-DD>.md`.
...
## Summary
Surgical fixes identified by /code-review.
...
Output path: `reports/code-review-impl-plan-<YYYY-MM-DD>.md`
...
- Apply chosen fixes. Output ready-to-run `/plan-work` commands for DESIGN items.
