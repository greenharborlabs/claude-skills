...
| --- | --- | --- | --- |
| Java/Gradle/Maven | `backend-security-reviewer-java` if security risk | `backend-api-contract-reviewer-java` if API files or public-api risk | standard `backend-reviewer-java` covers performance/concurrency |
| Python | `backend-security-reviewer-python` if security risk or sensitive files | `backend-api-contract-reviewer-python` if API/CLI files or public-api risk | standard `backend-reviewer-python` covers performance/concurrency |
| React/TypeScript | `frontend-reviewer` if security risk | `frontend-reviewer` if public-api risk | `frontend-reviewer` if performance/concurrency risk |
...
For React/TypeScript, use `frontend-reviewer` as a targeted specialist when
